...
def downloadImg(html, num, foldername):
    picpath ="/tmp/meizi/" +'%s' % (foldername)       #下载到的本地目录
    ...
    print "url:",myItems[0]
    if dic_meizi.get(md5(foldername)):
        dic_meizi.get(md5(foldername)).append(myItems[0])
    else:
        dic_meizi[md5(foldername)] =[myItems[0]]
